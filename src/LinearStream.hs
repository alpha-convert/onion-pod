...
import Data.Array.Polarized.Pull as Pull
import Data.Array.Polarized.Push as Push
import Data.Array.Mutable.Linear as LinArray
...
toPullArray :: Stream a %1 -> Pull.Array a
toPullArray (S (SF @s @a x0 next)) = 
    let arr = Pull.make undefined 0 in
    go x0 arr
    where
        go :: s %1 -> Pull.Array a %1 -> Pull.Array a
        go !x !arr =
            case next x of
                Done -> arr
                Skip x' -> go x' arr
